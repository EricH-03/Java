public class IntegerLinkedList {
	...
	public void doubleValues() {
		doubleValuesRecursiveHelper(head);
	}

	private void doubleValuesRecursiveHelper(IntegerNode n) {
		if (n == null) {
			return;
		} else {
			int valdoub = n.getData()*2;
			
			// set data in current node to double!
			n.setData(valdoub);
			
			// double each of the elements in the REST of the list
			doubleValuesRecursiveHelper(n.next);
		}
	}
	...
	public void doubleOddValues() {
		doubleOddValuesRecursiveHelper(head);
	}


	private void doubleOddValuesRecursiveHelper(IntegerNode n) {
		if (n == null) {
			return;
		} else {
			//was having issues with negative numbers so I added this neg checks

			int valdoub = n.getData();
		
			if(valdoub%2 != 0 || valdoub%-2 !=0){
				valdoub = valdoub*2;
			}
			...
			// double each of the elements in the REST of the list
			doubleOddValuesRecursiveHelper(n.next);
		}
	}
	...
	public boolean allNegative() {
		return allNegativeRecursiveHelper(head);
	}


	private boolean allNegativeRecursiveHelper(IntegerNode n) {
		if (n == null) {
			return true;}
		int value = n.getData();
		if(value>=0){
			...
			return allNegativeRecursiveHelper(n.next);
		}
	...
	public boolean isSortedAscending() {
		return isSortedAscendingRecursiveHelper(head, 0);
	}

	private boolean isSortedAscendingRecursiveHelper(IntegerNode n, int pos) {
		if (n == null || n.next == null ) {
			return true;}
		int first = n.getData();
		int next = n.next.getData();
		//System.out.println("First: " +first +" Next: " +next);
		if(first > next){
			return false;
		}
		else{
			return isSortedAscendingRecursiveHelper(n.next, pos+1);
		}
