...
class BinarySearchTree <K extends Comparable<K>, V>  {
	...
	public void insert (K key, V value) {
		...
		try{
			find(key);
			replace(key, value, root, null);
			/*if(l.left != null){
				while(key.compareTo(l.left.key) != 0 && l.left != null){
					l = l.left;
				}
			}
			...
			if(l.left != null){
				...
				while(key.compareTo(r.right.key) != 0){
					r = r.right;
	...
	private void insert(BSTNode<K,V> dad, BSTNode<K, V> l, K key, V value) {
		if(l == null){
			if(key.compareTo(dad.key)<0){
				dad.left = new BSTNode<K,V>(key, value);
				count++;
				return;
			}
			else{
				dad.right = new BSTNode<K,V>(key, value);
				count++;
				return;
			}
		}
		...
		if(l != null){
			if(key.compareTo(l.key)<0){
				insert(l, l.left, key, value);
			}
			else{
				insert(l, l.right, key, value);
			}
		}
	...
	public void replace (K key, V value, BSTNode<K,V> l,  BSTNode<K,V> parent) {
		if (l == null) {
			return;}
		...
		if (key.compareTo(l.key) == 0) {
			...
			replace(key, value, l.left, l);
			replace(key, value, l.right, l);
		}
	}
	...
	private V find(K key, BSTNode<K, V> l)throws KeyNotFoundException {
		if(l == null){
			throw new KeyNotFoundException();
		}

		if(l.key.compareTo(key) == 0){
			return l.value;
		}

		else if(l.key.compareTo(key)<0){
			return find(key, l.right);
		}

		else{
			return find(key, l.left);
		}
	...
	public int height() {
		...
		return Math.max(height(l.left),height(l.right));
		
	}

	private int height(BSTNode<K, V> l) {
		if(l == null){
			return 0;
		}
		return 1 + Math.max(height(l.left),height(l.right));
	}
	...
	public List<Entry<K,V>>	entryList() {
		...
		BSTNode<K,V> add = q.removeFirst();
		
		l.add(new Entry<K,V>(add.key, add.value));
