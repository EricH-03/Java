...
public class HashMap<K extends Comparable<K>, V> implements Map<K,V> {
	...
	public boolean containsKey(K key) {
		...
		while(iter.hasNext()){
			Entry<K, V> sigmacumlord = iter.next();
			if(sigmacumlord.key.equals(key)){
				return true;
			}
	...
	public List<Entry<K,V> >	entryList() {
		...
		Iterator<List<Entry<K,V>>> tableit = table.iterator();
	
	while(tableit.hasNext()){
		List<Entry<K,V>> list = tableit.next();
		Iterator<Entry<K,V>> iter = list.iterator();
		
		while(iter.hasNext()){
			Entry<K, V> sigmacumlord = iter.next();
			resultList.add(sigmacumlord);}
		}
	...
	while(iter.hasNext()){
		Entry<K, V> sigmacumlord = iter.next();
		if(sigmacumlord.getKey().equals(key)){
			sigmacumlord.setValue(value);
			return;
		}
