...
public class A7Tester {
	...
	public static void testSize() {
		...
		System.out.println("Eric testing of size, isEmpty");
		PriorityQueue pq1 = createNewPriorityQueue();

		displayResults (pq1.size() == 0, "size on empty Eric");
		displayResults (pq1.isEmpty(), "isEmpty on empty Eric");

		pq1.insert(1);
		displayResults (pq1.size() == 1, "size on empty Eric");
		displayResults (!pq1.isEmpty(), "isEmpty on empty Eric");
		
		pq1.insert(0);
		displayResults (pq1.size() == 2, "size on empty Eric");
		displayResults (!pq1.isEmpty(), "isEmpty on empty Eric");
		...
		displayResults (pq1.size() == 12, "size on empty Eric");
		displayResults (!pq1.isEmpty(), "isEmpty on empty Eric");
	}
	...
	public static void testInsertionAndRemoval() {
		...
		System.out.println("Eric testing of removeMin");
		PriorityQueue pq1 = createNewPriorityQueue();

		try{
		result = ((Integer)pq1.removeMin()).intValue();

	}catch(HeapEmptyException e){
			displayResults(true, "Eric Test Insert/Remove");
			displayResults(pq1.isEmpty(), "Eric Test Insert/Remove");
		}
		...
		result = ((Integer)pq1.removeMin()).intValue();
		// System.out.println("res: " + result + ":" + pq);
		displayResults(result == 1, "remove on multiple element PQ");
		displayResults(pq.size() == 0, "remove + size on multiple element PQ");
		displayResults(pq1.isEmpty(), "Eric Test Insert/Remove");
		...
		result = ((Integer)pq1.removeMin()).intValue();
		displayResults(result == 1, "insert + remove on multiple element PQ");
		displayResults(pq1.size() == 2, "insert + remove + size on multiple element PQ");

		result = ((Integer)pq1.removeMin()).intValue();
		displayResults(result == 2, "insert + remove on multiple element PQ");
		displayResults(pq1.size() == 1, "insert + remove + size on multiple element PQ");

		result = ((Integer)pq1.removeMin()).intValue();
		displayResults(result == 3, "insert + remove on 1 element PQ");
		displayResults(pq1.isEmpty(), "insert + remove + size on 1 element PQ");
